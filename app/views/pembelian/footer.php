...
<script>
    ...
    $(document).ready(function() {
        ...
        tablePembelian.on('order.dt search.dt', function() {
            tablePembelian.column(0, {
                search: 'applied',
                order: 'applied'
        ...
        $('.btnEditPembelian').click(function() {
            ...
            var jumlahpembelian = $(this).data('jumlahpembelian');
            var hargabeli = $(this).data('hargabeli');
            var idsupplier = $(this).data('idsupplier');
            var tanggalpembelian = $(this).data('tanggalpembelian');

            $('#editId').val(id);
            $('#editJumlahPembelian').val(jumlahpembelian);
            $('#editHargaBeli').val(hargabeli);
            $('#editTanggalPembelian').val(tanggalpembelian);

            $('#editIdBarang').val(idbarang).change();
            $('#editIdSupplier').val(idsupplier).change();
        });

        const hapusPembelian = document.getElementById('hapusPembelian');
        if (hapusPembelian) {
            $('.hapusPembelian').each(function(index) {
                $(this).on("click", function(e) {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        text: "Data pembelian akan terhapus dan stok barang akan berkurang seusai dengan jumlah pembelian",
                        icon: 'warning',
                        position: 'top',
